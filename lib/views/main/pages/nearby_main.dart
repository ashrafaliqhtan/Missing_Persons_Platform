      ...
      if (userReports is Map) {
        userReports.forEach((reportId, reportData) {
          ...
          if (reportData is Map) {
            ...
            // التحقق من الحالة والفلترة - التصحيح هنا
            ...
            if ((isVerified || status.isEmpty) && _passesFilters(report)) {
              try {
                ...
                print('معالجة التقرير: $fullName, الموقع: $location, الحالة: $status');
              ...
              print('التقرير غير مؤكد أو لا يطابق الفلاتر: $status');
            }
          }
        });
      }
...
bool _passesFilters(Map<dynamic, dynamic> report) {
  ...
  // التحقق من الحالة - التصحيح هنا
  final status = report['status']?.toString()?.toLowerCase() ?? '';
  final isVerified = status == 'verified' || status == 'مؤكد' || status == 'مفعل';
  
  if (!isVerified && status.isNotEmpty) {
    return false;
  }
    ...
    _reports.forEach((key, value) {
      if (value is Map) {
        value.forEach((key, value) {
          ...
          // التصحيح هنا - التحقق من الحالة
          final status = report['status']?.toString()?.toLowerCase() ?? '';
          if (status == 'verified' || status == 'مؤكد' || status == 'مفعل') {
            verified++;
          }
...
// تحديث دالة بناء قائمة التقارير
Widget _buildReportsList() {
  ...
  // التحقق من مرور التقرير عبر الفلاتر - الإصدار الأكثر مرونة
  bool _passesFilters1(Map<dynamic, dynamic> report) {
    // إذا لم يكن هناك تقرير، تخطى
    if (report.isEmpty) return false;
